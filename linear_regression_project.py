...
def scaleRow(M, r, scale):
    if scale==0:
        raise ValueError
    M[r]=[M[r][i]*scale for i in range(len(M[0]))]
    pass
...
def addScaledRow(M, r1, r2, scale):
    ...
    row=copy.deepcopy(M[r2])
    scale_row=[scale]*len(M[r2])
    row=[row[i]*scale for i in range(len(row))]

    # scaleRow(line,0,scale)
    M[r1]=[M[r1][j] +i for i,j in zip(row,range(len(row)))]
...
#     1 & -6 & 8 & -1 \\
#     0 & 1 & -\frac{17}{13} & \frac{9}{65} \\
#     0 & 1 &  -\frac{17}{13} & \frac{2}{13} \end{bmatrix}$
#     
#     
# $ --> \begin{bmatrix}
#     1 & -6 & 8 & -1 \\
